...
def findfwhmonbranches(
    ...
    # for h,spine in []:
        ...
        if len(branches) == 0:
            if printnumberofbranchesfound:
                print('\x1b[2K', end='\r')
                print(f"The number of branches found is {len(finalbranches)}, checking {h+1}", end='\r')
            continue
        correlations = []
        branchindexes = [[0]]
    ...
    for i,branch in enumerate(finalbranches):
        ...
        xs = [r*np.sin(phi)+ 298 for r,phi in zip(rs,phis)]
        ys = [r*np.cos(phi)+ 13 for r,phi in zip(rs,phis)]
