...
def findstreamers(row, maxvalnoise, hor=True):
...
def filtercamera2(image, settings, pastblurr = False):
    if not pastblurr:
        ...
        mean = np.mean(img)
        std = np.std(img)
        newerimg = np.array([[(10*(el-mean))/std if el > mean else 0 for el in row] for row in image])
    else:
...
if __name__ =='__main__':
    ...
    if settings['Camera'] == 1:
        ...
        for i, file in enumerate(files):
            writercamera2(file,i,[0],settings)
    else:
