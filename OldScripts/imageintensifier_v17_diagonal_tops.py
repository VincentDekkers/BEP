...
def findfwhmonbranches(image,
    ...
    try:
        ...
        if settings['NumberOfDirections'] == 1:
            ...
            image1 = np.array([findstreamers(row) for row in image]) #horizonal check
            image2 = np.array([findstreamers(row) for row in image.transpose()]).transpose() # vertical check
            ...
            image1 = np.array([findstreamers(row) for row in image]) #horizonal check:
            image2 = np.array([findstreamers(row) for row in image.transpose()]).transpose() # vertical check
            image3 = rotate45reverse([findstreamers(row) for row in rotate45(image)],shape)
            image4 = rotatereverse45reverse([findstreamers(row) for row in rotatereverse45(image)],shape)
