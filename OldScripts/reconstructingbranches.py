    ...
    for k,finalbranches in enumerate(branches):
        ...
        if tuple(finalbranches) == tuple([]):
            ...
            for branch in finalbranches:
                ...
                dist = weight = np.sqrt(distancesquared(branch[-1], (298, 13)))
                totweights += weight
                y = max([i[1] for i in branch])-13
                totfrac += (dist/y)*weight
            sideway.append(totfrac/totweights)
...
xvals = list(range(100, 1001, 10))*10
xvals.sort()
plt.scatter(xvals,data.flatten())
...
plt.title("5.73kV 100 mbar 500 ns")
ax = plt.gca()
