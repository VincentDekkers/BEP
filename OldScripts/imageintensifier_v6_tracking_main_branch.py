...
if __name__ == '__main__':
    ...
    for item in coordsoftops:
        if abs(item[0]-streamerstart) < 1 and len(spinalcoords) == 0:
            spinalcoords.append(item)
        elif len(spinalcoords) > 0:
            if (item[0]-spinalcoords[-1][0]-angle*(item[1]-spinalcoords[-1][1]))**2+(item[1]-spinalcoords[-1][1])**2 < 100:
                angle = rigidness*angle + (1-rigidness)*(item[0]-spinalcoords[-1][0])
                angleslist.append(angle)
                spinalcoords.append([((spinalcoords[-1][0]+angle*(item[1]-spinalcoords[-1][1]))+item[0])/2,item[1]])
                item[0] = 0
    smoothedangleslist = scipy.ndimage.gaussian_filter1d(angleslist,20)
    secondderivativesmoothedangleslist = [10**6*abs(smoothedangleslist[i+2]+smoothedangleslist[i]-2*smoothedangleslist[i+1]) for i in range(len(smoothedangleslist)-2)]
    topssecondderivativesmoothedangleslist = [i for i in range(len(secondderivativesmoothedangleslist)-1) if (secondderivativesmoothedangleslist[i]>secondderivativesmoothedangleslist[i-1]) and (secondderivativesmoothedangleslist[i]>secondderivativesmoothedangleslist[i+1]) and (secondderivativesmoothedangleslist[i]>50)]
    branchpoints = [el for i,el in enumerate(spinalcoords) if i in topssecondderivativesmoothedangleslist]
  
    plt.scatter(*zip(*spinalcoords))
    plt.scatter(*zip(*branchpoints))
    plt.plot(secondderivativesmoothedangleslist)
    for i,branchpoint in enumerate(branchpoints):
        maxdistance = 10**10
        ...
        for item in coordsoftops:
            if distancesquared(item,bestpoint) < 1 and len(spinalcoords) == 0:
                ...
                if (item[0]-spinalcoords[-1][0]-angle*(item[1]-spinalcoords[-1][1]))**2 < 200:
                    angle = rigidness*angle + (1-rigidness)*(item[0]-spinalcoords[-1][0])
                    angleslist.append(angle)
                    spinalcoords.append([((spinalcoords[-1][0]+angle*(item[1]-spinalcoords[-1][1]))+item[0])/2,item[1]])
        ...
        if len(spinalcoords)>20:
            plt.scatter(*zip(*spinalcoords))
    plt.scatter(*zip(*coordsoftops),marker='+',alpha=0.5)
