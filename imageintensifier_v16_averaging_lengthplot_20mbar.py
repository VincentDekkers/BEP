...
def findfwhmonbranches(image,
    ...
    try:
        ...
        for item in coordsoftops:
            if np.sqrt(distancesquared(item, spinalcoords2[-1])) - distanceparralel(angle, spinalcoords2[-1], item) < maxdistmainbrach and abs(calculateangle(spinalcoords2[-1],item) - angle) < maxbend and np.sqrt(distancesquared(item,spinalcoords2[-1]))<30:
...
if __name__ =='__main__':
    files = glob.glob('Metingen2025-06-23/*/*.ome.tif')
    allfiles = glob.glob('*') 
